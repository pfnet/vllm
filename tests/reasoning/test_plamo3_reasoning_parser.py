...
def test_non_streaming_no_begin_tag(tokenizer):
    # 先頭にBEGINタグが無い場合は全体がcontent扱い
    # (enable_thinking=False: reasoning分離を行わない)
...
def test_streaming_content_before_begin_tag(tokenizer):
    # enable_thinking=False: BEGINより前に通常テキストがある場合、
    # 以降も含めて全てcontent扱い
    parser = PlamoReasoningParser(
        tokenizer, chat_template_kwargs={"enable_thinking": False}
    )
...
def test_non_streaming_only_end_tag(tokenizer):
...
def test_default_kwargs_match_template_thinking_default(tokenizer, streaming):
    """Without chat_template_kwargs the parser must assume thinking is ON.

    PLaMo-3 chat templates default enable_thinking=True and inject the
    begin-think tag into the prompt, so the model output starts directly with
    reasoning. Regression test: the parser previously defaulted to False and
    streamed the reasoning (plus the literal END_THINK tag text) as content.
    """
    parser = PlamoReasoningParser(tokenizer)
    reasoning_text = "ユーザーの意図を確認する。"
    content_text = "回答です。"

    if streaming:
        deltas = [reasoning_text, END_THINK_TAG, content_text]
        reasoning, content = run_reasoning_extraction(parser, deltas, streaming=True)
    else:
        body = f"{reasoning_text}{END_THINK_TAG}{content_text}"
        reasoning, content = parser.extract_reasoning(body, request=None)  # type: ignore[arg-type]
...
def test_enable_thinking_disabled_delegates_to_identity(tokenizer):
    """enable_thinking=False must behave as the identity parser.

    is_reasoning_end must report True so the serving layer routes the whole
    stream to the tool parser, and streaming must pass deltas through as
    content without reasoning separation.
    """
    parser = PlamoReasoningParser(
        tokenizer, chat_template_kwargs={"enable_thinking": False}
    )

    assert parser.is_reasoning_end([100, 200]) is True
    assert parser.is_reasoning_end_streaming([100, 200], [200]) is True
    assert parser.extract_content_ids([100, 200]) == [100, 200]
    ...
    assert msg is not None and msg.content == "こんにちは" and msg.reasoning is None
