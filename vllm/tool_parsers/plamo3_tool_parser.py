...
class _ToolParserStreamState:
    """Positions and metadata for the incremental tool-call parser."""
...
class Plamo3ToolParser(ToolParser):
    ...
    def _emit_content_delta(
        ...
        """Append unstreamed content through ``until`` to ``output``."""
        state = self._stream
    ...
    def extract_tool_calls_streaming(
        ...
        Runs a text-based state machine over ``current_text`` to completion on
        each call, batching leading content, tool names, and arguments into a
        single DeltaMessage so a multi-token final delta cannot leave parsed
        output buffered.
        """
        state = self._stream
        output = _StreamDeltaBuilder()
